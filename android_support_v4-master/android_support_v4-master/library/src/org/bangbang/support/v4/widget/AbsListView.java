...
//        super.setFrame(left, top, right, bottom);
        changed = ReflectUtil.view_setFrame(AbsListView.class.getSuperclass().cast(View.class), AbsListView.class.getSuperclass().getSuperclass().getSuperclass(), left, top, right, bottom);
...
//        final boolean clipToPadding = (mGroupFlags & CLIP_TO_PADDING_MASK) == CLIP_TO_PADDING_MASK;
        final boolean clipToPadding = (ReflectUtil.getIntField(this, ViewGroup.class, "mGroupFlags")& CLIP_TO_PADDING_MASK) == CLIP_TO_PADDING_MASK;
        if (clipToPadding) {
...
//            mGroupFlags |= CLIP_TO_PADDING_MASK;
    ...
    private boolean startScrollIfNeeded(int deltaY) {
    	if (DEBUG) {
    		Log.d(TAG, "startScrollIfNeeded. deltaY: "+ deltaY);
    	}
...
//                        if ((mPersistentDrawingCache & PERSISTENT_SCROLLING_CACHE) == 0) {
    ...
    void trackMotionScroll(int deltaY, int incrementalDeltaY) {
    	if (DEBUG) {
    		Log.d(TAG, "trackMotionScroll. deltaY: " + deltaY + " incrementalDeltaY: " + incrementalDeltaY);
    	}
        final int childCount = getChildCount();
        if (childCount == 0) {
            return;
...
//            offsetChildrenTopAndBottom(incrementalDeltaY);
            ViewCompat.offsetChildrenTopAndBottom(this, incrementalDeltaY);
            invalidate();
